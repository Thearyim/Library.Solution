...
namespace Library.Models
{
    public class Author
    {
        ...
        public List<Book> GetBooks()
        {
            ...
            cmd.CommandText =
            @"SELECT books.id, books.title, copies.due_date
            FROM author 
            ...
            while (rdr.Read())
            {
                ...
                object bookDueDate = rdr.GetValue(2);
                DateTime? actualDueDate = null;
                if (bookDueDate != DBNull.Value)
                {
                    actualDueDate = (DateTime)bookDueDate;
                }

                Book foundBook = new Book(bookTitle, actualDueDate, id: thisBookId);
                books.Add(foundBook);
            }
